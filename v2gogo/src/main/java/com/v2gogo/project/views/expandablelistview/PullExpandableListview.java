...
import com.v2gogo.project.views.listview.refreshview.PullRefreshFooter;
import com.v2gogo.project.views.listview.refreshview.PullRefreshHeader;

/**
 * 下拉刷新上拉加载更多的ExpandableListview
 * 
 * @author houjun
 */
public class PullExpandableListview extends NoFageColorExpandableListview
{
	...
	private OnScrollListener mMyOnScrollListener = new OnScrollListener()
	{
		...
		public void onScrollStateChanged(AbsListView view, int scrollState)
		{
			...
			if (scrollState == SCROLL_STATE_IDLE)
			{
				if (!isRefreshing && !isLoagingMore && mEnableLoadMore
						&& PullExpandableListview.super.getLastVisiblePosition() == PullExpandableListview.super.getCount() - 1)
				{
					if (mOnLoadMoreListener != null)
					{
						isLoagingMore = true;
						mOnLoadMoreListener.onLoadMore(PullExpandableListview.this);
					}
				}
			}
		}
		...
		public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount)
		{
			if (mOnScrollListener != null)
			{
				...
				if (PullExpandableListview.super.getLastVisiblePosition() == totalItemCount - 1)
				{
