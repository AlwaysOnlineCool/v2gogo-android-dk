...
import com.v2gogo.project.views.refresh.PullToRefreshWebView;
 ...
 * @author houjun
 */
public class GroupGoodsDetailsActivityBack2 extends BaseActivity implements OnClickListener, IonItemClickCallback, IonReceiveTitleCallback,
		IonRetryLoadDatasCallback, IonSelectNumCallback
{
	...
	public void onInitViews()
	{
		...
		mPullToRefreshWebView = (PullToRefreshWebView) findViewById(R.id.webview_activity_webview);
		mProgressWebView = mPullToRefreshWebView.getRefreshableView();
		mProgressWebView.setVerticalScrollBarEnabled(false);
	}
	...
	public void clearRequestTask()
	{
		...
		mAbsoluteLayout.removeView(mPullToRefreshWebView);
			...
			mBarPopupWindow.setClickItemListener(new IonClickItemListener()
			{
				...
				public void onPopupWindowClick(int position)
				{
					...
					if (position == 2)
					{
						if (!V2GogoApplication.getMasterLoginState())
						{
							AccountLoginActivity.forwardAccountLogin(GroupGoodsDetailsActivityBack2.this);
						}
						if (null != mGoodsId)
						{
							ProfileCollectionsManager.AddCollectionsById(mGoodsId, ProfileCollectionsManager.COLLECTION_TYPE_PRODUCT,
									new IonAddCollectionsCallback()
									{
										...
										{
											ToastUtil.showConfirmToast(GroupGoodsDetailsActivityBack2.this, R.string.add_collections_tip);
										}

										@Override
										public void onAddCollectionsFail(String errorMessage)
										{
											ToastUtil.showAlertToast(GroupGoodsDetailsActivityBack2.this, errorMessage);
										}
									});
						}
					}
