...
import com.v2gogo.project.adapter.home.HomeAdapter2;
...
public class HomeActivity2 extends BaseActivity implements OnPullRefreshListener, OnLoadMoreListener, OnClickListener, IonClickGetCoinCallback,
		IonRetryLoadDatasCallback
{
	public SignGetCoinDialog mSignGetCoinDialog;

	private ProgressLayout mProgressLayout;
	private PullExpandableListview mPullExpendRefreshListView;
	...
	{
		return R.layout.home_activity_layout2;
	}
	...
	public void registerListener()
	{
		...
		mPullExpendRefreshListView.setLoadMoreEnable(true);
		mProgressLayout.setOnTryLoadDatasCallback(this);
		mPullExpendRefreshListView.setPullRefreshEnable(true);
		mPullExpendRefreshListView.setOnPullRefreshListener(this);
		mPullExpendRefreshListView.setOnLoadMoreListener(this);
	}
	...
	private void loadMoreDatas(int page)
	{
		PeopleConcernManager.getPeopleTopicList(page, new IonPeopleTopicListCallback()
		{
			...
			public void onPeopleTopicListSuccess(ConcernInfo concernInfo)
			{
				...
				if (mPullExpendRefreshListView != null)
				{
					boolean Flage = concernInfo.getCount() <= mPage;
					boolean isFinish = Flage ? true : false;
					if (isFinish)
					{
						mPullExpendRefreshListView.setLoadMoreEnable(false);
					}
					else
					{
						mPullExpendRefreshListView.setLoadMoreEnable(true);
			...
			{
				stopLoadmore();
				ToastUtil.showAlertToast(HomeActivity2.this, errorMessage);
			}
		});
	}
	...
	{
		if (mPullExpendRefreshListView.isLoadingMore())
		{
			mPullExpendRefreshListView.stopLoadMore();
		}
	}
	...
	private void loadNetHomeDatas()
	{
		...
		HomeManager.getAppHomeData(new IonHomeDataCallback()
		{
			...
			public void onHomeDataFail(String errorMessage)
			{
				...
				{
					if (mPullExpendRefreshListView.isRefreshing())
					{
						mPullExpendRefreshListView.stopPullRefresh();
					}
				}
			}
		});
	...
	private void initViews()
	{
		...
		mPullExpendRefreshListView = (PullExpandableListview) findViewById(R.id.profile_comment_list_expandable_listview);
	}
	...
	private void initHeaderView()
	{
		...
		mPullExpendRefreshListView.addHeaderView(headerView, null, false);
	...
	{
		mHomeAdapter = new HomeAdapter2(this,mPullExpendRefreshListView);
		mPullExpendRefreshListView.setAdapter(mHomeAdapter);
	}
			...
			mHeaderNavGridView.setOnItemClickListener(new OnItemClickListener()
			{
				...
				public void onItemClick(AdapterView<?> parent, View view, int position, long id)
				{
					...
					switch (navInfo.getContentType())
					{
						...
						case 2:// 百姓关注
							intent = new Intent(HomeActivity2.this, HomeConcernActivity.class);
							break;

						case 3:// 专题
								// intent = new Intent(HomeActivity.this, HomeLiveActivity.class);
							intent = new Intent(HomeActivity2.this, HomeSubjectActivity.class);
						...
						case 4:// 内链接
							try
							{
								...
								InternalLinksTool.jump2Activity(HomeActivity2.this, type, srcId, url, null);
							}
							catch (Exception e)
							{
							}
							return;
						case 5:// 外部链接
							InternalLinksTool.forward2Website(HomeActivity2.this, navInfo.getUrl());
							return;
						case 6:// 全民秀
							intent = new Intent(HomeActivity2.this, HomeV2gogoActivity.class);
							break;
						default:
							break;
					}
