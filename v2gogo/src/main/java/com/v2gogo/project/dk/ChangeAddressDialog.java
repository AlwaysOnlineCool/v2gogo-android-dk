...
import com.v2gogo.project.dk.wheel.widget.adapters.AbstractWheelTextAdapter;
import com.v2gogo.project.dk.wheel.widget.views.OnWheelChangedListener;
import com.v2gogo.project.dk.wheel.widget.views.OnWheelScrollListener;
import com.v2gogo.project.dk.wheel.widget.views.WheelView;
...
public class ChangeAddressDialog extends Dialog implements android.view.View.OnClickListener {
	...
	private AddressTextAdapter areasAdapter;

	private String strProvince = "贵州省";
	private String strCity = "贵阳市";
	private String strArea = "云岩区";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initAreas(mAreasDataMap.get(strCity));
		areasAdapter = new AddressTextAdapter(context, arrAreas, getAreaItem(strArea), maxsize, minsize);
		wvAreas.setVisibleItems(5);
		wvAreas.setViewAdapter(areasAdapter);
		...
		wvCitys.addChangingListener(new OnWheelChangedListener() {
			...
			public void onChanged(WheelView wheel, int oldValue, int newValue) {
				...
				String[] areas = mAreasDataMap.get(currentText);
				initAreas(areas);
				areasAdapter = new AddressTextAdapter(context, arrAreas, 0, maxsize, minsize);
				wvAreas.setVisibleItems(5);
				wvAreas.setViewAdapter(areasAdapter);
		...
		wvAreas.addChangingListener(new OnWheelChangedListener() {
			...
			public void onChanged(WheelView wheel, int oldValue, int newValue) {
				// TODO Auto-generated method stub
				String currentText = (String) areasAdapter.getItemText(wheel.getCurrentItem());
				strArea = currentText;
				setTextviewSize(currentText, areasAdapter);
	...
	public void initAreas(String[] areas) {
		if (areas != null) {
			...
			String[] city = mAreasDataMap.get("贵阳市");
			arrAreas.clear();
			int length = city.length;
			for (int i = 0; i < length; i++) {
				arrAreas.add(city[i]);
